...
/**
 * Base class of repository of Model\ArticleVote document.
 */
abstract class ArticleVoteRepository extends \Mongator\Repository
{
    ...
    {
        $this->documentClass = 'Model\ArticleVote';
        $this->isFile = false;
        $this->collectionName = 'model_articlevote';
    ...
    public function fixMissingReferences($documentsPerBatch = 1000)
    {
        $skip = 0;
        do {
            $cursor = $this->getCollection()->find(array('article' => array('$exists' => 1)), array('article' => 1))->limit($documentsPerBatch)->skip($skip);
            $ids = array_unique(array_values(array_map(function ($result) { return $result['article']; }, iterator_to_array($cursor))));
            if (count($ids)) {
                $collection = $this->getMongator()->getRepository('Model\Article')->getCollection();
                ...
                if ($idsDiff = array_diff($ids, $referenceIds)) {
                    $this->remove(array('article' => array('$in' => $idsDiff)), array('multiple' => 1));
                }
            }
        ...
        do {
            ...
            if (count($ids)) {
                $collection = $this->getMongator()->getRepository('Model\User')->getCollection();
                $referenceCursor = $collection->find(array('_id' => array('$in' => $ids)), array('_id' => 1));
                $referenceIds =  array_values(array_map(function ($result) { return $result['_id']; }, iterator_to_array($referenceCursor)));
