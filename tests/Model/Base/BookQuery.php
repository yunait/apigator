...
/**
 * Base class of query of Model\Book document.
 */
abstract class BookQuery extends \Mongator\Query\Query
{
     ...
     *
     * @return Model\BookQuery The query with added criteria (fluent interface).
     */
    ...
    public function findByComment($value)
    {
        ...
        return $this->mergeCriteria(array('comment' => $castValue ));
    }

    /**
     * Find by "isHere" field.
     *
     * @param mixed $value The value.
     *
     * @return Model\BookQuery The query with added criteria (fluent interface).
     */
    public function findByIsHere($value)
    {
        $castValue = (boolean) $value;
        if ($castValue !== $value) throw new \Exception('Bad value: type boolean expected');
        
        return $this->mergeCriteria(array('isHere' => $castValue ));
    }
}