 ...
 */
abstract class IdGeneratorSingleInheritanceGrandParentQuery extends \Mongator\Query\Query
{
            ...
            if (isset($data['type'])) {
                if ('parent' == $data['type']) {
                    if (!isset($identityMaps['parent'])) {
                        $identityMaps['parent'] = $mongator->getRepository('Model\IdGeneratorSingleInheritanceParent')->getIdentityMap()->allByReference();
                    }
     ...
     *
     * @return Model\IdGeneratorSingleInheritanceGrandParentQuery The query with added criteria (fluent interface).
     */
