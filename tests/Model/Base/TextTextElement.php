...
/**
 * Base class of Model\TextTextElement document.
 */
abstract class TextTextElement extends \Model\TextElement
{
     ...
     *
     * @return \Model\TextTextElement The document (fluent interface).
     */
    public function setDocumentData($data, $clean = false)
    {
        ...
        if (isset($data['text_text'])) {
            $this->data['fields']['text_text'] = (string) $data['text_text'];
        }
    ...
    /**
     * Set the "text_text" field.
     *
     * @param mixed $value The value.
     *
     * @return \Model\TextTextElement The document (fluent interface).
     */
    public function setText_text($value)
    {
        if (!isset($this->data['fields']['text_text'])) {
            if (!$this->isNew()) {
                $this->getText_text();
                if (
                    ( is_object($value) && $value == $this->data['fields']['text_text'] ) ||
                    ( !is_object($value) && $value === $this->data['fields']['text_text'] )
                ) {
                    return $this;
                }
                ...
                $this->fieldsModified['text_text'] = null;
                $this->data['fields']['text_text'] = $value;
                return $this;
            }
        } elseif (
            ( is_object($value) && $value == $this->data['fields']['text_text'] ) ||
        ...
        if (!isset($this->fieldsModified['text_text']) && !array_key_exists('text_text', $this->fieldsModified)) {
            $this->fieldsModified['text_text'] = $this->data['fields']['text_text'];
        } elseif (
            ( is_object($value) && $value == $this->fieldsModified['text_text'] ) ||
            ( !is_object($value) && $value === $this->fieldsModified['text_text'] )
        ) {
            unset($this->fieldsModified['text_text']);
        }
    ...
    /**
     * Returns the "text_text" field.
     *
     * @return mixed The $name field.
     */
    public function getText_text()
    {
        $this->addFieldCache('text_text');

        if (!isset($this->data['fields']['text_text']) &&
            !$this->isFieldInQuery('text_text'))
        {
            $this->loadFull();
        }
        // Still not set? It can only be null
        if (!isset($this->data['fields']['text_text'])) {
            $this->data['fields']['text_text'] = null;
        }

        return $this->data['fields']['text_text'];
    }
    ...
    public function set($name, $value)
    {
        ...
        if ('text_text' == $name) {
            return $this->setText_text($value);
        }
    ...
    public function fromArray(array $array)
    {
        ...
        if (isset($array['text_text'])) {
            $this->setText_text($array['text_text']);
        }
    ...
    public function toArray($withReferenceFields = false)
    {
        ...
        $array['text_text'] = $this->getText_text();
    ...
    public function queryForSave()
    {
        ...
        if (isset($this->data['fields'])) {
            if ($isNew || $reset) {
                if (isset($this->data['fields']['text_text'])) {
                    $query['text_text'] = (string) $this->data['fields']['text_text'];
                }
            } else {
                if (isset($this->data['fields']['text_text']) || array_key_exists('text_text', $this->data['fields'])) {
                    $value = $this->data['fields']['text_text'];
                    $originalValue = $this->getOriginalFieldValue('text_text');
                    if ($value !== $originalValue) {
                        if (null !== $value) {
                            $query['$set']['text_text'] = (string) $this->data['fields']['text_text'];
                        } else {
                            $query['$unset']['text_text'] = 1;
                        }
                    }
                }
            }
